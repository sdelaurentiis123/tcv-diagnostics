"""Network-free tests for required B4 evaluation W&B wrapping."""
...
from paper0.tools.run_b4_pde_refiner_evaluation_wandb import (
    evaluation_metrics,
...
def test_wrapper_locks_b4_identity_mode_and_heldout_paths(tmp_path: Path) -> None:
    args = _args(tmp_path)
...
def test_wrapper_extracts_final_and_stage_metrics() -> None:
    aggregate = {
    ...
    generation = {
        "target_count": 4,
        "final_forecast": {"shape": [4, 32, 1, 5, 64, 32, 88]},
        "stage_forecast": {"shape": [4, 4, 4, 5, 64, 32, 88]},
        "wall_seconds": 4.0,
    ...
    assert metrics["forecast/final_ensemble_size"] == 32
    assert metrics["forecast/stage_prefix_size"] == 4
    assert metrics["field/equal_channel_fair_crps"] == 0.6
    assert metrics["stage_repair/level3/E_transport"] == 8.0
