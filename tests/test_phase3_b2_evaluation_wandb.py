...
from paper0.tools.run_b2_evaluation_wandb import (
    evaluation_metrics,
...
def test_wandb_wrapper_locks_identity_and_never_accepts_held_out_paths(
    tmp_path: Path,
) -> None:
    args = _args(tmp_path)
    command = validate_evaluation_command(args)
    assert command[-6:] == args.evaluation_args[1:]
...
def test_wandb_wrapper_extracts_compact_authoritative_metrics() -> None:
    aggregate = {
