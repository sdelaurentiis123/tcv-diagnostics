"""Safety locks for complete frozen R1 O1 transport finalization."""
...
LAUNCHER = ROOT / "cluster/phase2_matched_o1_finalize_r1.sbatch"
MANIFEST = ROOT / "paper0/manifests/phase2_matched_o1_r1_transport_finalization.json"
RECONSTRUCTION = (
    ROOT / "paper0/results/phase2_matched_o1_reconstruction_r1_6894345.json"
)
CANDIDATE_PHI = (
    ROOT / "paper0/results/phase2_matched_e6b_candidate_phi_r1_6894377.json"
)
LOCKED = (
...
def test_finalize_launcher_is_clean_cpu_only_rocky9_bash() -> None:
    ...
    for required in (
        ...
        '"R2_launch_allowed": (len(runs) == 6 and not r1_accepted)',
        '"O2_launch_allowed": r1_accepted',
    ):
...
def test_transport_manifest_matches_both_frozen_matrices() -> None:
    ...
    assert decision["r2_required_if_any_checkpoint_fails"] is True
    assert decision["r2_launch_allowed_before_all_six_complete"] is False


def test_finalize_launcher_runs_all_results_before_r2_decision() -> None:
    text = LAUNCHER.read_text(encoding="utf-8")
    launch = text.index("for run_index in 0 1 2 3 4 5; do")
    wait = text.index("if ! wait", launch)
    summarize = text.index('"R2_launch_allowed"', wait)
