"""Safety and provenance locks for frozen R2 E6B exact-phi recovery."""
...
LAUNCHER = ROOT / "cluster/phase2_matched_e6b_candidate_phi_r2.sbatch"
MANIFEST = ROOT / "paper0/manifests/phase2_matched_e6b_candidate_phi_r2.json"
RECONSTRUCTION = (
...
LOCKED = (
    ...
    ROOT / "paper0/manifests/phase2_o1_codec_r2_runs.json",
...
def test_candidate_phi_manifest_matches_frozen_reconstruction() -> None:
    ...
    assert manifest["identity"]["codec"] == "dcae_l10"
    expected = [
    ...
    assert output["o2_launch_allowed_from_this_stage_alone"] is False
    assert output["authoritative_transport_finalization_required_next"] is True


def test_candidate_phi_launcher_uses_all_frozen_r2_candidate_hashes() -> None:
    text = LAUNCHER.read_text(encoding="utf-8")
    manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
    for run in manifest["runs"]:
        assert run["reconstruction_result"]["sha256"] in text
        assert run["training_candidate"]["sha256"] in text
        assert run["validation_candidate"]["sha256"] in text
