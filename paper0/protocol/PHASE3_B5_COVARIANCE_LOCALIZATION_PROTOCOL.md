...
- the already generated H1 training forecast for targets `[2,432)`, job
  `6901393`, SHA-256
...
The legacy training residual audit was generated before the present protocol
made the constant-potential gauge convention explicit. Its stored spatial and
cross-field sufficient statistics therefore cannot be transformed exactly
into a sample-wise gauge-fixed training reference: the required per-sample
potential means were not retained. Using those statistics directly against
new gauge-fixed validation objects would not be a like-for-like comparison.

The complete H1 training forecast generated by that same audit still exists as
an immutable, hash-locked artifact. This protocol therefore authorizes reading
...
`phi` separately for every training and validation truth, H1 forecast, B5
member, and B5 ensemble mean. Then subtract an axisymmetric bias
`mean(target,z)` from each realized H1 residual and the validation innovation
before computing fluctuation correlations. B5 anomalies already have zero
conditional ensemble mean; only numerical closure is checked. Transport uses
the original physical fields because the authoritative radial `E cross B`
operator is invariant to a constant potential offset.
