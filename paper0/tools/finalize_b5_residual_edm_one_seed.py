#!/usr/bin/env python3
"""Apply the frozen B5 seed-1701 one-step gate without rescoring."""
...
from tcv_diagnostics.b5_residual_edm_acceptance_gate import (  # noqa: E402
    B5_COMPARATOR_SCOPE,
    ...
    B5_SCORE_SCOPE,
    B5_TRAINING_SCOPE,
    evaluate_b5_one_seed_acceptance,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--inherited-b3-manifest", type=Path, required=True)
    parser.add_argument("--inherited-b3-manifest-sha256", required=True)
...
def main() -> None:
    ...
    paths = (
        ...
        args.inherited_b3_manifest,
        args.training_wandb,
        args.evaluation_wandb,
        args.output_directory,
    )
    if any("85606" in str(path).lower() for path in paths):
        raise ValueError("B5 gate paths may not mention held-out 85606")
    for path in paths:
        assert_development_path(path)
    verify_checkout(args.gate_execution_commit)
    evaluation_path = verify_input(
        args.evaluation_result,
        args.evaluation_result_sha256,
        "B5 evaluation result",
    )
    comparator_path = verify_input(
        args.comparator_result,
        args.comparator_result_sha256,
        "B5 frozen comparator",
    )
    ...
    b3_manifest_path = verify_input(
        args.inherited_b3_manifest,
        args.inherited_b3_manifest_sha256,
        "B5 inherited B3 manifest",
    )
    training_wandb_path = verify_input(
        args.training_wandb,
        args.training_wandb_sha256,
        "B5 training W&B record",
    )
    evaluation_wandb_path = verify_input(
        args.evaluation_wandb,
        args.evaluation_wandb_sha256,
        "B5 evaluation W&B record",
    )
    if (
        args.comparator_result_sha256 != B5_COMPARATOR_SHA256
        or args.evaluation_manifest_sha256 != B5_MANIFEST_SHA256
        or args.inherited_b3_manifest_sha256 != EXPECTED_B3_MANIFEST_SHA256
    ):
        raise RuntimeError("B5 gate frozen authority hash differs")
    output = Path(args.output_directory)
    if output.exists():
        raise FileExistsError(f"refusing to overwrite B5 gate result {output}")
    output.mkdir(parents=True)

    result = load_strict_json(evaluation_path)
    comparator = load_strict_json(comparator_path)
    manifest = load_strict_json(manifest_path)
    b3_manifest = load_strict_json(b3_manifest_path)
    training_wandb = load_strict_json(training_wandb_path)
    evaluation_wandb = load_strict_json(evaluation_wandb_path)
    if result.get("scope") != B5_EVALUATION_SCOPE:
        raise ValueError("B5 gate evaluation-result scope differs")
    if comparator.get("scope") != B5_COMPARATOR_SCOPE:
        raise ValueError("B5 gate comparator scope differs")
    if (
        result.get("evaluation_manifest", {}).get("sha256")
        != args.evaluation_manifest_sha256
    ):
        raise ValueError("B5 gate evaluation manifest provenance differs")
    if "gates" not in b3_manifest:
        raise ValueError("B5 inherited B3 numerical gates are absent")
    ...
    if training.get("scope") != B5_TRAINING_SCOPE:
        raise ValueError("B5 gate training scope differs")
    if generation.get("scope") != B5_GENERATION_SCOPE:
        raise ValueError("B5 gate generation scope differs")
    if score.get("scope") != B5_SCORE_SCOPE:
        raise ValueError("B5 gate score scope differs")
    if result.get("forecast", {}).get("sha256") != score.get(
        "forecast_artifact", {}
    ).get("sha256"):
        raise ValueError("B5 gate forecast/score identity differs")
    if result.get("training_commit") != training.get("paper0_commit"):
        raise ValueError("B5 gate training commit provenance differs")

    acceptance = evaluate_b5_one_seed_acceptance(
        result=result,
        score=score,
        training=training,
        ...
        manifest=manifest,
        inherited_b3_gates=b3_manifest["gates"],
    ...
    status = (
        "completed_passed_frozen_B5_one_seed_gate"
        if passed
        else (
            "completed_failed_joint_physics_after_field_family_pass"
            if marginal_joint_failure
            else "completed_failed_frozen_B5_one_seed_gate"
        )
    )
    final = {
        "schema_version": 1,
        "scope": "phase3_B5_residual_EDM_seed1701_scientific_gate_85604",
        "status": status,
        ...
        "inherited_b3_manifest_input": {
            "path": str(b3_manifest_path),
            "sha256": args.inherited_b3_manifest_sha256,
        },
