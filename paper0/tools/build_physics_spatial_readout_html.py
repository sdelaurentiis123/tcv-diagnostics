...
<section id="spatial"><p class="label">06 · Ben’s simultaneous spatial proposal</p><h2>Two localized density views contain real information about an unobserved region</h2><p class="lede">This first simultaneous reconstruction test contains no temporal model. It asks only whether sparse measurements A and B, taken at the same instant, can reconstruct the full fluctuation state and predict a spatially separated diagnostic C. The observations are BES/GPI-like box averages—not faithful experimental forward models.</p><figure><div class="figure-image"><img alt="Truth observed diagnostics reconstruction error and heldout channels for simultaneous spatial reconstruction" src="__FIG_S0_HERO__"></div><figcaption><span class="fignum">Figure 5.</span> A closed-form ridge/Wiener map is fit chronologically on training frames only. The displayed frame is the median held-out-C error case. The result establishes nonlocal simultaneous information; it does not establish causal forecasting or experimental realism.</figcaption></figure><figure><div class="figure-image"><img alt="Spatial reconstruction skill versus distance toroidal mode and plasma region" src="__FIG_S0_SCALE__"></div><figcaption><span class="fignum">Figure 6.</span> Held-out-C error falls below the no-fluctuation baseline, correlation remains nonzero at large separation, and recovery is sharply band-limited. In n=20–35 the retained-power ratios are __BAND_DETAIL__; at n≥40 they are only __HIGH_DETAIL__.</figcaption></figure></section>

<section id="meaning"><p class="label">07 · What is supported now</p><h2>A clean scientific readout, with the boundary kept explicit</h2><table><thead><tr><th>Supported by these figures</th><th>Not supported yet</th></tr></thead><tbody><tr><td>Large-scale one-step field motion is forecastable to a useful degree.</td><td>No current ensemble has passed the full transport-covariance gate.</td></tr><tr><td>Transport errors can be localized by position, separation, and physical mode.</td><td>The apparent diagnostic ranking from earlier ETKF tests is not yet definitive.</td></tr><tr><td>Sparse simultaneous density measurements contain nonlocal low/mid-mode information.</td><td>The spatial test is not a faithful BES/GPI instrument model and is not cross-shot validation.</td></tr><tr><td>Higher-mode spatial fluctuations are poorly reconstructed by the linear reconstruction baseline.</td><td>85606, assimilation, sensor ranking, and steering remain outside this readout.</td></tr></tbody></table><div class="callout"><strong>Immediate next decision.</strong> Review the plotted transport surface and synthetic footprints with Ben. The bounded variogram-loss campaign is implemented but paused at the user’s request; no training arm is active in this report.</div><p class="small">Forecast extraction status: __REPORT_STATUS__. All forecast visuals use model-training seed 1702, target __TARGET__, and the fixed 32-member seed banks. The spatial reconstruction uses frames [0,432) for training, [432,496) as the untouched guard, and [496,624) for validation.</p></section>
