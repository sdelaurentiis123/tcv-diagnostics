#!/usr/bin/env python3
"""Run the B4 evaluator inside one required live online W&B run."""
...
def validate_evaluation_command(args: argparse.Namespace) -> tuple[list[str], str]:
    ...
    if sha256_path(evaluator) != args.evaluator_sha256:
        raise ValueError("B4 PDE-Refiner evaluator SHA-256 differs")
    values = list(args.evaluation_args)
    if values and values[0] == "--":
        values = values[1:]
    if not values or any("85606" in value.lower() for value in values):
        raise ValueError("B4 evaluation command is empty or mentions held-out data")
    if Path(_one_argument(values, "--output-directory")) != args.output_directory:
        raise ValueError("B4 wrapper/evaluator output directories differ")
    if _one_argument(values, "--paper0-commit") != args.paper0_commit:
        raise ValueError("B4 wrapper/evaluator Paper 0 commits differ")
    if int(_one_argument(values, "--seed")) != args.seed:
        raise ValueError("B4 wrapper/evaluator seeds differ")
    mode = _one_argument(values, "--mode")
    if mode not in ("smoke", "full"):
        raise ValueError("B4 evaluator mode differs")
...
def evaluation_metrics(
    ...
    aggregate = final_score["field_and_marginal_calibration"]["regions"][
        "eligible_union"
    ]["aggregate"]
    levels = stage_score["stagewise_repair"]["levels"]
    metrics: dict[str, int | float | bool] = {
        "evaluation/stage": 2,
        "evaluation/completed": 1,
        "forecast/target_count": int(generation["target_count"]),
        "forecast/final_ensemble_size": int(
            generation["final_forecast"]["shape"][1]
        ),
        "forecast/stage_prefix_size": int(
            generation["stage_forecast"]["shape"][1]
        ),
    ...
    for level in levels:
        ...
        for name in ("E_field", "E_power", "E_real", "E_cross", "E_transport"):
            metrics[f"stage_repair/level{index}/{name}"] = float(level[name])
    return metrics
...
def main() -> None:
    ...
    spec = WandbRunSpec(
        ...
        job_type="phase3_b4_pde_refiner_evaluation",
        tags=(
            "paper0",
            "phase3",
            "b4",
    ...
    final_score = load_strict_json(output / "score_final_M32.json")
    stage_score = load_strict_json(output / "score_stages_M4.json")
    expected_scope = (
    ...
    if (
        ...
        or result.get("truth_opened_only_after_both_forecast_hashes") is not True
    ):
        run.finish(exit_code=1)
        raise RuntimeError("completed B4 evaluation identity differs")
    metrics = evaluation_metrics(result, generation, final_score, stage_score)
    run.log(metrics, step=1)
    run.summary.update(
        {
            **metrics,
            "provenance/result_sha256": sha256_path(output / "result.json"),
            "provenance/final_forecast_sha256": result["final_forecast"][
                "sha256"
            ],
            "provenance/stage_forecast_sha256": result["stage_forecast"][
                "sha256"
            ],
            "provenance/final_score_sha256": result["final_score"]["sha256"],
            "provenance/stage_score_sha256": result["stage_score"]["sha256"],
            "provenance/scientific_seed_bank_sha256": result[
    ...
    if str(remote.state) != "finished":
        raise RuntimeError("remote W&B B4 evaluation run is not finished")
    tracking = {
        "schema_version": 1,
        "required": True,
        "mode": "online",
