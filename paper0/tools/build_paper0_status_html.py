...
<nav aria-label="Contents"><div><a href="#answer">One-minute answer</a><a href="#data">Data</a><a href="#codec">Representation</a><a href="#models">Models</a><a href="#physics">Physics figures</a><a href="#covariance">Covariance</a><a href="#causes">Causes</a><a href="#current">Current run</a><a href="#claims">Claims</a></div></nav>
<main>
...
<section id="physics"><p class="kicker">05 · Physics figures</p><h2>Flux, modes, phase, and spatial covariance</h2><p class="lede">These are the physical diagnostics behind the gate—not only aggregate ML scores. The transport trace below is an O1 codec reconstruction. The B5 panels are one-step forecasts. An autonomous Paper 0 rollout is intentionally absent because O4 has not been authorized.</p><figure><div class="figimg"><img alt="Truth and codec reconstruction separatrix transport time series" src="__FIG_FLUX_TRACE__"></div><figcaption><span class="fignum">Figure 5.</span> Actual confined-separatrix particle and internal-energy flow through the simulated one-fifth wedge. f8 tracks the net series closely even while its local signed face map fails—direct evidence that integration can hide spatial error.<span class="source">PHASE2_O1_TRANSPORT_READOUT.md · codec reconstruction, not a forecast rollout</span></figcaption></figure><figure><div class="figimg"><img alt="B5 toroidal power, realization coherence, spread skill and coverage" src="__FIG_SPECTRAL__"></div><figcaption><span class="fignum">Figure 6.</span> B5 generates approximately the right power for 13/15 field-band combinations, but only 4/15 land in the correct next-frame realization. No material mode band has calibrated power uncertainty.<span class="source">PHASE3_B5_READOUT.md · n=5k because zperiod=5</span></figcaption></figure><figure><div class="figimg"><img alt="B5 density and pressure cross-field phase and coherence diagnostics" src="__FIG_CROSSFIELD__"></div><figcaption><span class="fignum">Figure 7.</span> Mean density/pressure–potential phase is good, but high-band coherence and every real/imaginary cross-spectrum calibration projection fail. Mean phase alone is not enough for nonlinear flux.<span class="source">PHASE3_B5_READOUT.md · member-wise complex cross-spectra</span></figcaption></figure><figure><div class="figimg"><img alt="Truth residual and B5 anomaly spatial autocorrelation" src="__FIG_SPATIAL_ACF__"></div><figcaption><span class="fignum">Figure 8.</span> B5 anomaly correlation lengths differ systematically from true H1 residual structure along nonperiodic and toroidal directions. This is the spatial organization defect seen before transport integration.<span class="source">PHASE3_B5_COVARIANCE_LOCALIZATION_READOUT.md</span></figcaption></figure><figure><div class="figimg"><img alt="Truth residual and B5 anomaly toroidal power covariance" src="__FIG_TOROIDAL_COV__"></div><figcaption><span class="fignum">Figure 9.</span> Mode-resolved anomaly covariance exposes where matching marginal amplitude still fails to reproduce the true coupled toroidal innovation.<span class="source">PHASE3_B5_COVARIANCE_LOCALIZATION_READOUT.md</span></figcaption></figure></section>

<section id="covariance"><p class="kicker">06 · The decisive B5 finding</p><h2>Local variance is present; coherent transport variance is not</h2><p class="lede">B5 is not simply too narrow everywhere. It is close to calibrated for pooled local face-flux contributions and badly underdispersed after those same contributions are integrated.</p><figure><div class="figimg"><img alt="B5 local versus integrated transport calibration" src="__FIG_TRANSPORT__"></div><figcaption><span class="fignum">Figure 10.</span> Local transport spread–skill is near one, while integrated separatrix spread–skill is only 0.413–0.485. The defect is spatial, modal, and cross-field covariance—not a missing global multiplier.<span class="source">PHASE3_B5_READOUT.md · job 6901587</span></figcaption></figure><figure><div class="figimg"><img alt="B5 separatrix covariance localization" src="__FIG_COVARIANCE__"></div><figcaption><span class="fignum">Figure 11.</span> Scalar inflation sufficient to repair integrated spread would overdisperse the local field by about 2.1–2.4×. The ensemble must reorganize covariance, not merely increase amplitude.<span class="source">PHASE3_B5_COVARIANCE_LOCALIZATION_READOUT.md · job 6901914</span></figcaption></figure></section>

<section id="causes"><p class="kicker">07 · Cause localization</p><h2>The transition is conditional and not exactly equivariant</h2><p class="lede">Phase 3.5 rejects the broad claim that stochastic emulation is impossible. It rejects only one fixed, global, condition-independent linear residual law.</p><figure><div class="figimg"><img alt="Phase 3.5 evidence ranking and representation transfer" src="__FIG_PHASE35__"></div><figcaption><span class="fignum">Figure 12.</span> Strong evidence supports symmetry defects, drift, state dependence, retained-state limitations, and memory. Effective sample size is not the primary explanation. Fourier coordinates improve later-block variance capture from 21.9% to 39.4% at the largest matched budget, but do not solve every transport dependency.<span class="source">phase3_5/PHASE3_5_DECISION_MEMO.md · job 6907468</span></figcaption></figure></section>

<section id="current"><p class="kicker">08 · What is running</p><h2>ECRD tests the mechanisms implicated by the evidence</h2><p class="lede">The ladder adds deep raw-field conditioning, exact toroidal handling in the generator, a symmetrized H1 parent plus mean correction, joint multiscale noise, and then the smallest history extension.</p><figure><div class="figimg"><img alt="ECRD intervention matrix and live Rusty status" src="__FIG_ECRD__"></div><figcaption><span class="fignum">Figure 13.</span> All 12 model runs completed 100 epochs and 10,800 optimizer updates. At this snapshot, __COMPLETED__/12 M32 evaluations are complete, __RUNNING__ are running, and __PENDING__ are queued. Training objectives differ across mean-head arms and are not a scientific ranking.<span class="source">ECRD_MODEL_DEVELOPMENT_PROTOCOL.md · array 6913512 · reducer 6913520</span></figcaption></figure><div class="verdict warning"><strong>Next decision:</strong> the frozen seven-family reducer checks fair CRPS, spectral retention, Ne–phi dependence, spatial transport covariance, integrated spread without local overdispersion, and multi-seed robustness. Only a passing arm may open one evaluation on 85606.</div></section>

<section id="claims"><p class="kicker">09 · Scientific boundary</p><h2>What we can and cannot say</h2><div class="columns"><div><h3>Supported</h3><ul><li>There is coherent toroidal motion and slow chronological drift.</li><li>The matched codec can preserve the evaluated representation.</li><li>Low field error and mean cross-phase do not guarantee transport-faithful joint forecasts.</li><li>B5's dominant defect is covariance organization, not zero spread.</li></ul></div><div><h3>Not supported</h3><ul><li>No model has yet passed the complete Paper 0 forecast gate.</li><li>No autonomous rollout has been accepted.</li><li>85606 generalization remains unknown.</li><li>ETKF/EnKF, diagnostic ranking, and steering remain unauthorized.</li></ul></div></div></section>
</main>
...
<script>const b=document.querySelector('.theme');b.addEventListener('click',()=>{const r=document.documentElement;const d=r.dataset.theme==='dark'||(!r.dataset.theme&&matchMedia('(prefers-color-scheme: dark)').matches);r.dataset.theme=d?'light':'dark';});</script>
    ...
    replacements = {
        ...
        "__FIG_FLUX_TRACE__": figures["flux_trace"],
        "__FIG_SPECTRAL__": figures["spectral"],
        "__FIG_CROSSFIELD__": figures["crossfield"],
        "__FIG_SPATIAL_ACF__": figures["spatial_acf"],
        "__FIG_TOROIDAL_COV__": figures["toroidal_cov"],
...
def main() -> None:
    ...
    figures = {
        ...
        "flux_trace": png_uri(args.repo / "paper0/figures/phase2_o1_transport/codec-separatrix-transport-time-series.png"),
        "spectral": png_uri(args.repo / "paper0/figures/phase3_b5/b5-field-spectral-localization.png"),
        "crossfield": png_uri(args.repo / "paper0/figures/phase3_b5/b5-cross-field-localization.png"),
        "spatial_acf": png_uri(args.repo / "paper0/figures/phase3_b5_covariance_localization/b5-covariance-spatial-acf.png"),
        "toroidal_cov": png_uri(args.repo / "paper0/figures/phase3_b5_covariance_localization/b5-covariance-toroidal-power.png"),
