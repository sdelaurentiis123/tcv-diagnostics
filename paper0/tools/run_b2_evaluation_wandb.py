#!/usr/bin/env python3
"""Run one B2 evaluator inside a required live online W&B run."""
...
def validate_evaluation_command(args: argparse.Namespace) -> list[str]:
    ...
    if sha256_path(evaluator) != args.evaluator_sha256:
        raise ValueError("B2 evaluator SHA-256 differs")
    values = list(args.evaluation_args)
    if values and values[0] == "--":
        values = values[1:]
    if not values or any("85606" in value.lower() for value in values):
        raise ValueError("B2 evaluation command is empty or mentions held-out data")
    if Path(_one_argument(values, "--output-directory")) != args.output_directory:
        raise ValueError("B2 wrapper/evaluator output directories differ")
    if _one_argument(values, "--paper0-commit") != args.paper0_commit:
        raise ValueError("B2 wrapper/evaluator Paper 0 commits differ")
    if int(_one_argument(values, "--seed")) != args.seed:
        raise ValueError("B2 wrapper/evaluator seeds differ")
...
def main() -> None:
    ...
    command = validate_evaluation_command(args)
    spec = WandbRunSpec(
        ...
        job_type="phase3_b2_probabilistic_evaluation",
        tags=("paper0", "phase3", "b2", "evaluation", "85604-only"),
    )
    import wandb
    ...
    if (
        ...
        run.finish(exit_code=1)
        raise RuntimeError("completed B2 evaluation identity differs")
    ...
    if str(remote.state) != "finished":
        raise RuntimeError("remote W&B evaluation run is not finished")
    tracking = {
        "schema_version": 1,
        "required": True,
        "mode": "online",
