"""B3 identity wrapper around the byte-locked B2 scientific metric engine.

The B3 protocol deliberately reuses the exact field, calibration, spectral,
cross-field, and member-wise transport calculations already used for B2.  This
module adds no numerical metric.  It verifies the frozen source hashes, checks
the B3 artifact identity, delegates once, and relabels only result provenance.
"""
...
def verify_locked_metric_sources() -> dict[str, str]:
    """Fail closed if any pre-B3 metric implementation has changed."""
    ...
    for relative, expected in LOCKED_METRIC_SOURCES.items():
        ...
        if actual != expected:
            raise RuntimeError(
                f"frozen B3 metric source differs for {relative}: {actual}"
            )
        verified[relative] = actual
    return verified


def _validate_fgn_artifact_identity(forecast_artifact: Any) -> None:
    metadata = forecast_artifact.metadata
    if (
        forecast_artifact.model_seed != 1701
        or metadata.get("source_kind") != "selected_B3_FGN"
        or metadata.get("arm") != "B3-FGN-H1"
        ...
        raise ValueError("B3 FGN forecast identity differs")
...
def _relabel(
    ...
    *,
    ...
    result = dict(score)
    result["scope"] = (
        "bounded_non_scientific_B3_FGN_H1_evaluator_smoke_scoring_85604"
        if bounded_smoke
        else "B3_FGN_H1_truth_separated_probabilistic_scoring_85604"
    )
    result["model_arm"] = "B3-FGN-H1"
...
def _score_fgn_forecast(
    *,
    ...
    _validate_fgn_artifact_identity(forecast_artifact)
...
def score_fgn_forecast(
    *,
    ...
    """Score the full B3 M32 artifact with unchanged B2 numerical metrics."""

    return _score_fgn_forecast(
        catalog=catalog,
        forecast_artifact=forecast_artifact,
        native_truth=native_truth,
        geometry=geometry,
